...
#Inner useful variables to isolate key/value pairs made of css selectors only.
...
try:
    #Light scraping
    if sys.argv[1] == "-l":
        inputL = [url for url in scrape_env["url"]] #Creating inner List to merge the lists returned by extract_url()
        urls_list = []
        for innerUrl in inputL:
            innerL = extract_url(innerUrl)
            urls_list += innerL #Merging the Lists returned by extract_url() together
        for url in urls_list:
            print("Scraping url: ", url)
            scraped_page = requests.get(url) #Request to the url
            scraped_page_soup  = bs4.BeautifulSoup(scraped_page.text, "html.parser") #Creating Soup
            print("--------------------")
            for box in scraped_page_soup.select(scrape_env["box"]): #For loop to gather data from every box
                for selector in selectors_dict:
                    if selectors_dict[selector][1] == "getText": #Establish where InnerText or tag attribute is to be scraped
                        print(selector, box.select_one(selectors_dict[selector][0]).getText())
                        df.loc[i, keys[0]] = url
                        df.loc[i, selector] = box.select_one(selectors_dict[selector][0]).getText() #Insert data into dataframe
                    else:
...
df.to_csv("scraped_data.csv") #Producing output sheet
